...
- **Leaderboard reworked to per-game + all-time.** The old Today/All-time toggle was confusing (and "today" shows 0 outside the event week, since no game is unlocked yet). Replaced with a scrollable chip row: All-time (cumulative) plus one board per game, each ranking only the guests who played that game on the same 0–1000 scale, which is a fairer head-to-head. Open question: Connections is listed as an offline in-person finale in CLAUDE.md but the app still scores it; its chip currently shows (and would just be empty during the real event). Decide whether Connections stays a scored game or is excluded from the board.
